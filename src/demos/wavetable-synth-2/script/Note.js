...
export class Note {
  constructor(noteData) {
    ...
    this.filterAttack = 0.02;
    this.filterDecay = 0.5;
    this.ampAttack = 0.02;
    this.ampDecay = 0.5;
    this.width = 0.5;
  ...
  play(semitone, octave, time) {
    ...
    const cutoffRate = Math.pow(
      2,
      (CUTOFF_OFFSET_CENTS * this.filterCutoff) / 1200
    );
    const startFrequency = Math.min(
      nyquistFrequency,
      cutoffRate * this.pitchFrequency
    );
    const filterAmountRate = Math.pow(
      2,
      (FILTER_MOD_OFFSET_CENTS * this.filterAmount) / 1200.0
    );
    const filterAmountFrequency = Math.min(
      nyquistFrequency,
      filterAmountRate * startFrequency
    );

    this.biquad.frequency.setValueAtTime(startFrequency, time);
    this.biquad.frequency.setTargetAtTime(
      filterAmountFrequency,
      time,
      this.filterAttack
    );
    this.biquad.frequency.setTargetAtTime(
      startFrequency,
      time + this.filterAttack,
      this.filterDecay
    );
