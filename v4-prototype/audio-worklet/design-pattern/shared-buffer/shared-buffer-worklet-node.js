...
class SharedBufferWorkletNode extends AudioWorkletNode {
  ...
  constructor(context, options) {
    ...
    // Worker backend URL resolved relative to this module.
    const workerUrl =
      new URL('shared-buffer-worker.js', import.meta.url).href;
    this._worker = new Worker(workerUrl);

    // This node is a messaging hub for the Worker and AWP. After the initial
    // setup, message passing between worker and process is rarely necessary
    // because of the SharedArrayBuffer.
  ...
  _onWorkerInitialized(eventFromWorker) {
    ...
    console.warn(
      `[SharedBufferWorklet] Unknown message: ${eventFromWorker}`
    );
  }

  /**
   * Handles the initial event from the associated processor.
  ...
  _onProcessorInitialized(eventFromProcessor) {
    ...
    console.warn(
      `[SharedBufferWorklet] Unknown message: ${eventFromProcessor}`
    );
  }
}
