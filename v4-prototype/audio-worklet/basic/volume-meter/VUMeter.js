...
class VUMeter {
   ...
   * @param {AnalyserNode} [analyserNode] AnalyserNode for audio data.
  ...
  constructor(
    ...
    this.analyser_ = actualAnalyser;
    if (this.analyser_) {
      this.analyser_.fftSize = actualFftSize;
      this.dataArray_ =
  ...
  draw(volume) {
    ...
    // Optional background fill when not transparent
    ...
    // Green VU bar
    this.canvasContext_.fillStyle = '#22c55e';
    ...
    this.canvasContext_.strokeStyle = '#64748b';
    this.canvasContext_.lineWidth = 1;
    this.canvasContext_.globalAlpha = 0.3;
    this.canvasContext_.beginPath();
    for (let p = 0.2; p < 1.0; p += 0.2) {
      this.canvasContext_.moveTo(0, this.height_ * p);
      this.canvasContext_.lineTo(this.width_, this.height_ * p);
    }
