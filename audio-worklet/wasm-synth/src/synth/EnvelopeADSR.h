...
#define MIN_DURATION (1.0 / 100000.0)
    ...
    /**
     * Sustain level. Range 0.0 to 1.0.
     *
     * @param level
     */
    void setSustainLevel(synth_float_t level){
        mSustainLevel = level;
    }

    synth_float_t getSustainLevel() {
        return mSustainLevel;
    }

    void setReleaseTime(synth_float_t time){
        mRelease = time;
    }

    synth_float_t getReleaseTime() {
        return mRelease;
    }
...
private:
    ...
    void startDecay() {
        ...
        if (duration < MIN_DURATION) {
            ...
            mScaler = SynthTools::convertTimeToExponentialScaler(duration, kSampleRate);
