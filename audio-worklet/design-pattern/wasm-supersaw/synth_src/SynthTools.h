...
public:
    ...
    static void addBuffers(const synth_float_t *source,
                           synth_float_t gain,
                           synth_float_t *destination,
                           int32_t numSamples) {
        for (int i = 0; i < numSamples; i++) {
    ...
    /**
     * Maps a MIDI value into a specified range.
     */
    static synth_float_t interpolateMIDIValue(
        uint8_t midiValue, synth_float_t min, synth_float_t max) {
        return (synth_float_t)(midiValue) / 127.0f * (max - min) + min;
    }
};
