...
function GlObject(shader, vertices, colors, normals) {
  ...
  var vertexArray = new CanvasFloatArray(vertices);
  var colorArray = new CanvasUnsignedByteArray(colors);
  this.vertexOffset = 0;
  this.colorOffset = vertexArray.alignedSizeInBytes();
  this.normalOffset = this.colorOffset + colorArray.alignedSizeInBytes();
  var sizeInBytes = this.normalOffset;
  var normalArray = null;
  if (normals != undefined) {
    normalArray = new CanvasFloatArray(normals);
    sizeInBytes += normalArray.alignedSizeInBytes();
  }
