...
const initialize = async () => {
  ...
  wavetableView1.setComplexData(WavetableDataSet[wavetableInitialIndex].real,
      WavetableDataSet[wavetableInitialIndex].imag);
  const wavetableView2 = document.querySelector('#wave-view-2');
  wavetableView2.setComplexData(WavetableDataSet[wavetableInitialIndex].real,
      WavetableDataSet[wavetableInitialIndex].imag);
  ...
  wavetableSelect1.addEventListener('select', (event) => {
    ...
    if (event.detail.value) {
      ...
      wavetableView1.setComplexData(WavetableDataSet[index].real,
  ...
  wavetableSelect2.addEventListener('select', (event) => {
    ...
    if (event.detail.value) {
      const index = event.detail.value;
      wavetableView2.setComplexData(WavetableDataSet[index].real,
          WavetableDataSet[index].imag);
    }
  });
};
